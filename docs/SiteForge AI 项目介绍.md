...
![SiteForge AI 完整流程演示](./images/siteforge-ai/demo-walkthrough.mp4)

视频（约 3 分钟，2 倍速）完整记录了一次真实任务的执行过程：输入自然语言需求，生成并确认 3 份视觉预览，随后进入 AI 实现、生产构建、公网部署与健康检查，最终返回可访问的网站 URL。

> 配图与演示视频均使用真实产品前端与脱敏演示数据生成，不包含用户账号、密钥或真实业务数据。
...
- Runner 每 30 秒从公网检查自己的 `/health`；即使隧道进程仍存活，域名出现 1016、5xx 或持续不可达也会自动回收并换址；
...
- 新地址建立后由 Runner 使用受信任 token 主动登记，控制面从公网再次核对实例编号、健康状态和部署能力后才写入 D1；
- 连接类错误时页面仍展示“修复连接”，作为自动恢复失败时的人工兜底；
- 用户点击后，浏览器从本机 Runner 取得新地址；控制面从公网核对实例编号、健康状态和部署能力，通过后登记并自动按原模式重新派发任务。

**结果：** 终端或 Codex 会话退出不会再带走 Runner；临时域名失效会被公网监控发现，新地址自动登记到控制站，不再依赖重新部署环境变量或用户先点按钮，同时仍明确 Quick Tunnel 不具备生产 SLA。
