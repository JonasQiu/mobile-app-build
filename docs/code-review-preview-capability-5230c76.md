...
存在 2 项非阻断风险：Runner 中立构建门禁已将外层测试时限由 240 秒提高到 500 秒，但 CI 在冷缓存或资源竞争下的耗时可靠性仍需持续观察；新增自动化以纯函数和源码契约检查为主，真实加载失败、快速操作竞态、触屏与读屏仍需产品验收闭环。
...
### N1：中立构建集成测试存在环境时限敏感性，外层预算已由 240 秒提高到 500 秒

- **位置**：`packages/codegen/tests/build.test.mjs:20-22`；相关阶段时限见 `packages/codegen/src/build.js:8,51-58,94-103`。
- **实现正确性**：单文件复跑 `tests/build.test.mjs` 为 2/2 通过，中立 fixture 实际耗时约 262 秒；没有断言失败或构建失败。该证据支持 `runBuild` 在中立项目上的实现正确性。
- **CI 超时可靠性**：此前全套测试在 240 秒预算下取消，同一 fixture 的成功耗时又超过 240 秒，说明取消属于外层门禁时限假阴性，而不是功能回归。低速磁盘、冷缓存或并发构建仍可能显著放大耗时。
- **处置**：外层测试预算由 240 秒提高到 500 秒，覆盖当前约 262 秒实测，并为安装、构建、启动和清理保留余量。调整后在允许 Turbopack 绑定本地端口的环境复跑为 2/2 通过、0 失败、0 取消，总耗时约 65 秒（热缓存）；后续仍应分别记录安装/构建阶段耗时，并观察 CI 冷缓存分位数。
- **收口复验**：在同步后的远端基线 `8f7fc66` 上，以 500 秒预算重跑完整 `packages/codegen` 套件，48/48 通过、0 失败/取消，总耗时 202.26 秒；其中中立 fixture 为 201.27 秒。`node scripts/check-docs.mjs` 同期检查 27 份文档通过，`git diff --check` 通过。
- **严重度判断**：非阻断。实现正确性证据通过；500 秒调整降低当前 CI 假阴性风险，但不把环境耗时波动误判为实现失败。
...
2. 发布门禁需跟踪 N1 在 500 秒预算下的耗时分布；若仍取消，应按环境/门禁可靠性处理，而不是把取消误报为本功能断言回归。
