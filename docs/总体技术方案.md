...
- D1 的 `runner_endpoints` 保存 Runner 最新心跳地址、实例编号和待处理换址请求；45 秒内的登记地址优先于环境变量回退地址。
...
- Runner 可为自身控制 API 维护 Quick Tunnel，每 10 秒登记当前地址；隧道退出或控制面收到“修复连接”请求后会自动换址，恢复后页面重派原任务。
