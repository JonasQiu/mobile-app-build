...
flowchart LR
  ...
  K --> V[3 SVG Preview Options]
  S --> A[(D1 Preview Approval)]
  A --> R
...
sequenceDiagram
  ...
  R->>R: 生成 3 份需求专属 SVG 预览
  R-->>W: approval_required + preview checkpoint
  W->>D: 保存 awaiting_approval / pending
  W-->>U: 展示多图；不占 Runner 名额
  alt 用户要求换一组
    U->>W: step preview + regeneratePreview
    W->>R: 仅失效 preview 及下游检查点
    R-->>U: 新 preview set
  end
  U->>W: 确认 previewId
  W->>R: 读取当前 preview artifacts 并校验 ID
  W->>D: 保存 approved + previewSetId
...
- Codex 生成前必须存在需求哈希一致的 preview checkpoint，并且 D1 已保存、Runner 已复核当前 preview set 中的 selectedPreviewId。浏览器显示状态不能替代该门禁。
...
- D1 保存 requirement、status、currentStage、previewUrl，以及 `project_preview_approvals` 中的 pending/approved、previewSetId 和 selectedPreviewId。
...
- `GET /api/projects` 为 building/ready/awaiting_approval 等项目拉取 Runner，并在可信终态时更新 D1；等待预览确认不计入两任务并发上限。
- 暂停由控制面鉴权后转发给 Runner；Runner 使用 job 专属 `AbortController` 终止当前子进程并回写 `paused`。继续、重跑或单步均分配新 job id，旧 job 事件按 id 隔离。
- `continue` 从首个未完成阶段的保存位置继续；首次运行和 `rerun` 都在 preview 后停止等待新确认。`step preview + regeneratePreview` 仅清除预览及下游标记，不触发 Codex；其他单步对成功目标直接复用、对失败目标原地续修。完整部署检查点复用时保持原 URL。
