...
当前版本已经移除关键词模板、健身示例项目、前端计时假进度、示例日志和站内 `/preview` 伪交付。执行页每 15 秒同步 Runner 真实消息，运行中的任务可暂停，暂停、失败和已交付任务可重新执行；历史记录支持删除，每用户最多同时执行 2 个需求。只有 Mobile Spec、生产构建、部署和公网健康检查全部通过，项目才进入 `delivered`。
...
- 真实暂停会向 Runner 发送中断信号并终止当前 Codex、Mobile Spec、安装、构建或健康检查子进程；重新执行会创建新 job 并清除旧交付状态。
...
- 持久源码 checkpoint、ZIP 下载、取消、断点继续、持久事件库、正式部署 Provider 尚未实现。
