#!/usr/bin/env node
/**
 * mobile-spec SDD 观察兜底 hook（observe）—— 纯工具级被动观测
 *
 * 定位：**机制强制兜底层**，补全主动埋点（mobile-spec monitor，由 skills 显式调）的漏埋。
 *   - 主动埋点（phase 级）由各 Mobile Spec stage skill 在 phase 边界显式调 `mobile-spec monitor`，
 *     走公司 eval-emit → skillshub（权威骨架）。
 *   - 本 hook 由 Claude Code / Codex hooks 运行时触发，
 *     不依赖 LLM，记工具级/产物级被动事件（source: mobile-spec-observe，落本地 jsonl），
 ...
 * `MOBILE_SPEC_OBSERVE_FORWARD_URL` 时，经 curl push（与 monitor 的公司 eval-emit 链路
 * 物理隔离，互不影响）。项目索引见 ~/.mobile-spec/obs/projects.json。
 * MOBILE_SPEC_VERSION 为占位符，install-hooks.js 拷贝时替换。
 */
...
const MOBILE_SPEC_OBSERVE_TAG = 'mobile-spec-observe';
const MOBILE_SPEC_VERSION = '__MOBILE_SPEC_VERSION__';
// 默认不上报；显式设置 MOBILE_SPEC_OBSERVE_FORWARD_URL 才会经 curl push 到自建后台。
// 与 monitor 的公司 eval-emit 链路物理隔离，改这里不影响公司埋点。
const FORWARD_URL = process.env.MOBILE_SPEC_OBSERVE_FORWARD_URL || '';

// ────────────────────────────── 公共维度 ──────────────────────────────

/**
 * observe 数据根：~/.mobile-spec/obs/（MOBILE_SPEC_HOME_OVERRIDE 可重定向，与 monitor.js 一致）。
 * 事件不落业务项目，统一放用户级目录，按项目 cwd 哈希分子目录隔离。
 */
function getObsHome() {
  const home = process.env.MOBILE_SPEC_HOME_OVERRIDE || os.homedir();
  return path.join(home, '.mobile-spec', 'obs');
}
...
/**
 * 返回某项目的 observe 目录：~/.mobile-spec/obs/<hash>/，并在 ~/.mobile-spec/obs/projects.json
...
/**
 * 幂等登记项目索引：~/.mobile-spec/obs/projects.json = { "<hash>": { cwd, updated_at } }。
 ...
 * monitor session binding；只有已被识别为 Mobile Spec 的 session 才允许降级到项目 current。
 */
...
function baseEvent(ctx, extra) {
  const ev = {
    ts: nowIso(),
    source: MOBILE_SPEC_OBSERVE_TAG,
    mobile_spec_version: MOBILE_SPEC_VERSION,
...
function parseStagePrompt(prompt) {
  ...
  const xmlCommand = text.match(
    /<command-(?:message|name)>\s*\/?mobile-spec-(proposal|design|task|coding|verify|archive|change)\s*<\/command-(?:message|name)>/i
  );
  if (xmlCommand) {
  ...
  const direct = text.match(/^\s*[$/]?mobile-spec-(proposal|design|task|coding|verify|archive|change)\b/i);
...
function handleEvent(payload) {
  ...
  if (name === 'PostToolUse') {
    ...
    if (['Write', 'Edit', 'MultiEdit'].includes(tool)) {
      ...
      // mobile-spec monitor 负责（时机精确到"开始创建产物/创建结束"）。observe 只记被动观测事件
...
function forward(event) {
  // 默认不上报（仅落本地）。显式设置 MOBILE_SPEC_OBSERVE_FORWARD_URL 才经 curl push 到自建后台。
  if (!FORWARD_URL) return;
  if (process.env.MOBILE_SPEC_OBSERVE_DISABLE_FORWARD === '1') return;
  try {
