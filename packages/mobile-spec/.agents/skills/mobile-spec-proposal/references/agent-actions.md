...
3. 临时 summary 不得写入业务仓库、`openspec/` 或 `~/.mobile-spec/workflow/`。
4. 调用 `mobile-spec workflow complete-agent-action --change <change> --hook <hook> --action <id> --result <pass|failed|skipped> --summary-file <temp-json> --json` 回写。
