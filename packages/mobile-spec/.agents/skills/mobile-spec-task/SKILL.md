---
name: mobile-spec-task
description: 基于已通过的 Mobile Spec 设计和项目上下文生成 tasks.md 与 Coding 收口命令。用于 task 拆解阶段；不要用于直接编码。
license: MIT
---
...
1. 执行 `preStage --stage task`；顶层 ok 且 blocking actions 完成后，执行 `mobile-spec workflow plan --stage task --change <change> --json` 核对依赖、artifacts、路径和 `review.md` gate。
