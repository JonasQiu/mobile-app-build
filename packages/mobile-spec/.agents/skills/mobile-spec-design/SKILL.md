---
name: mobile-spec-design
description: 生成 Mobile Spec design 与 review 并通过设计 gate。用于技术方案阶段；不要用于需求提案或编码。
license: MIT
---
...
1. 执行 `preStage --stage design`；顶层 ok 且 blocking actions 完成后，执行 `mobile-spec workflow plan --stage design --change <change> --json` 核对依赖、artifacts、路径和 gate。
