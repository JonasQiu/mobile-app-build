...
function getMobileSpecHome() {
  const home = process.env.MOBILE_SPEC_HOME_OVERRIDE || os.homedir();
  return path.join(home, '.mobile-spec');
}
...
function getWorkflowHome() {
  return path.join(getMobileSpecHome(), 'workflow');
}
...
/**
 * 把旧版业务项目内的 Mobile Spec sidecar 迁到用户目录。
 *
...
function migrateLegacyWorkflowState(projectRoot) {
  ...
  migrateTree(
    path.join(openspecDir, '.mobile-spec'),
  ...
  if (fs.existsSync(changesDir)) {
    for (const entry of fs.readdirSync(changesDir, { withFileTypes: true })) {
      ...
      migrateTree(
        path.join(changesDir, entry.name, '.mobile-spec'),
  ...
  removeDirIfEmpty(path.join(openspecDir, '.mobile-spec'));
  return result;
}
...
function migrateArchivedChanges(projectRoot, archiveDir, result) {
  ...
  for (const entry of fs.readdirSync(archiveDir, { withFileTypes: true })) {
    ...
    migrateTree(
      path.join(archiveDir, entry.name, '.mobile-spec'),
