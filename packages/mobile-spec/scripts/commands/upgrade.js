 ...
 *
 * 升级**全局 CLI 包**（@mobile-app-build/mobile-spec 本身）到最新版。
 *
 * 与 mobile-spec update 的边界（关键区分）：
 *   - mobile-spec update  —— 刷新**项目资产**（schema / agents / config / hooks），不碰 CLI 二进制
 *   - mobile-spec upgrade —— 升级**全局 CLI 二进制**（@mobile-app-build/mobile-spec 包），不碰项目资产
 *                     （完成后仅提示用户手动跑 mobile-spec update 刷新项目资产）
 *
 * 流程：
 ...
 *   6. 提示用户手动跑 mobile-spec update 刷新项目资产
 */
...
/**
 * 解析参数：mobile-spec upgrade [--pm <pnpm|npm|yarn>] [--dry-run] [-f|--force]
 *
...
function parseUpgradeArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if ((arg === '--pm') && args[i + 1]) {
      ...
      // upgrade 不接受位置参数；忽略避免误用（如 mobile-spec upgrade ./path）
    }
  }

  if (pm !== null && !ALLOWED_PMS.includes(pm)) {
    console.error(
      `[mobile-spec] 非法 --pm 值: "${pm}"。可选值：${ALLOWED_PMS.join(', ')}`
    );
    process.exit(1);
  }
...
function detectPackageManager(scriptPath, env) {
  ...
  if (norm.includes('lib/node_modules/@mobile-app-build/mobile-spec') || norm.includes('node_modules/@mobile-app-build/mobile-spec')) {
    // 排除已判过的 pnpm 路径后，含 node_modules/@mobile-app-build/mobile-spec 视为 npm 全局
    return 'npm';
  }
...
function getInstallCommand(pm) {
  switch (pm) {
    case 'pnpm':
      return { cmd: 'pnpm', args: ['add', '-g', '@mobile-app-build/mobile-spec@latest'] };
    case 'yarn':
      // yarn classic 全局安装；berry 已弃用 global，仍用 classic 写法
      return { cmd: 'yarn', args: ['global', 'add', '@mobile-app-build/mobile-spec'] };
    case 'npm':
    default:
      return { cmd: 'npm', args: ['install', '-g', '@mobile-app-build/mobile-spec@latest'] };
  }
}
...
function getCurrentVersion(scriptPath) {
  ...
  if (sp) {
    try {
      // bin/mobile-spec.js → ../package.json
...
async function cmdUpgrade(args, opts = {}) {
  ...
  console.log(`\nmobile-spec upgrade  (pm: ${pm})  当前版本: ${current}\n`);
  ...
  console.log('\n升级完成。');
  console.log('💡 如需刷新当前项目资产（schema / agents / config），请运行：mobile-spec update');
}
