...
async function checkForCliUpdate(command, opts = {}) {
  ...
  try {
    ...
    stderr.write(
      `[mobile-spec] 检查新版本失败：${detail}；继续执行 mobile-spec ${command}。\n`
    );
  ...
  const accepted = await confirm(
    {
      message:
        `发现 @mobile-app-build/mobile-spec 新版本 ${current} → ${latest}，` +
        `是否现在自动安装？`,
      default: true,
    },
  ...
  try {
    ...
    const detail = (err && err.message) || String(err);
    stderr.write(
      `[mobile-spec] 自动升级失败：${detail}；继续执行 mobile-spec ${command}。\n`
    );
  ...
  stdout.write(
    `\n[mobile-spec] 升级完成。` +
    `\x1b[1;33m请重新执行 \`mobile-spec ${command}\`。\x1b[0m\n`
  );
