/**
 * mobile-spec obs —— 本地查看 SDD 可观测数据
 *
 * 子命令：
 *   mobile-spec obs list                    列出所有接入过 mobile-spec 的项目（路径 + 事件数 + 最后更新）
 *   mobile-spec obs show [项目]             单个项目的汇总视图（phase/产物/docs/review）
 ...
 * 路径隔离：复用 MOBILE_SPEC_HOME_OVERRIDE。
 */
...
function renderList(projects) {
  if (!projects.length) {
    return ['[mobile-spec] 暂无可观测项目。', '', '提示：接入项目后运行 SDD 流程，可观测数据会自动写入 ~/.mobile-spec/。'].join('\n');
  }
  const lines = ['[mobile-spec] 可观测项目（共 ' + projects.length + ' 个）:', ''];
  ...
  lines.push('');
  lines.push('查看详情: mobile-spec obs show <序号或项目路径>');
  return lines.join('\n');
}
...
async function cmdShow(args) {
  ...
  if (arg) {
    ...
    if (!project) {
      console.error(`[mobile-spec] 未找到项目: ${arg}`);
      console.error('运行 mobile-spec obs list 查看可用项目。');
      process.exitCode = 1;
      return;
    }
  } else {
    // 交互式选择
    if (!projects.length) {
      console.error('[mobile-spec] 暂无可观测项目。');
      process.exitCode = 1;
      return;
    }
    try {
      ...
      if (!project) {
        console.error('[mobile-spec] 未选择项目。');
        process.exitCode = 1;
        return;
      }
    } catch (e) {
      if (e && e.message === 'no-prompt-dep') {
        console.error('[mobile-spec] 非交互环境或缺少交互库。请指定项目: mobile-spec obs show <序号或路径>');
        console.error('运行 mobile-spec obs list 查看可用项目。');
        process.exitCode = 1;
        return;
      }
  ...
  process.stdout.write(`[mobile-spec] 已生成并打开 obs 看板: ${htmlFile}\n`);
}
...
function cmdObsHelp(sub) {
  if (sub === 'list') {
    return `Usage: mobile-spec obs list [--json]

列出所有接入过 mobile-spec 的项目（路径 + 事件数 + 需求数 + 最后更新时间）。

Options:
  --json    输出结构化 JSON`;
  }
  if (sub === 'show') {
    return `Usage: mobile-spec obs show [项目] [--json]

打开单个项目的 SDD 可观测 HTML 看板（总分/分项/docs 文件级使用/review 门禁）。

参数:
  项目      项目路径子串 / obs hash / 序号（见 mobile-spec obs list）
            不带参数时交互式选择。

Options:
  --json    输出结构化 JSON，不打开 HTML`;
  }
  return `Usage: mobile-spec obs <command> [options]
...
数据位置:
  observe:  ~/.mobile-spec/obs/<hash>/events.jsonl
  monitor:  ~/.mobile-spec/monitor/state/repos/<repoKey>/requirements/*.json
  （MOBILE_SPEC_HOME_OVERRIDE 可重定向两处根目录）

Examples:
  mobile-spec obs list
  mobile-spec obs show                    # 交互式选择
  mobile-spec obs show ~/my-project       # 按路径
  mobile-spec obs show 1                  # 按序号
  mobile-spec obs show --json`;
}
