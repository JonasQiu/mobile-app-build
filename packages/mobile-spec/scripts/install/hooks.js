/**
 * mobile-spec 观察兜底 hook 安装器（install-hooks）
 *
 * 由 `mobile-spec init / update` 调用，把**机制强制兜底层**接入目标项目：
 ...
 *      在 UserPromptSubmit /
 *      PostToolUse / Stop 下追加 mobile-spec 兜底项；保留用户已有 hooks，按 command 中的
 *      `mobile-spec-observe` 标记去重（重复 update 只替换本工具的项）。
 *
 * 这是主动埋点（mobile-spec monitor）的**机制强制兜底**：PostToolUse 由 Claude Code 运行时
...
function getAgentsRoot() {
  return process.env.MOBILE_SPEC_AGENTS_ROOT_OVERRIDE || path.resolve(__dirname, '..', '..', '.agents');
}
...
function stripMobileSpecHooks(arr) {
  const out = [];
  for (const entry of arr) {
    ...
    const kept = hs.filter(
      (h) => !(h && typeof h.command === 'string' && h.command.includes(MOBILE_SPEC_OBSERVE_TAG))
    );
...
function installHooks(targetPath, opts = {}) {
  ...
  if (!fs.existsSync(srcHook)) {
    throw new Error(`[mobile-spec] 观察脚本不存在：${srcHook}（请检查 .agents/hooks/）`);
  }
  let hookText = fs.readFileSync(srcHook, 'utf8');
  hookText = hookText.replace(/__MOBILE_SPEC_VERSION__/g, getVersion());
  const destinations = [
  ...
  const configs = [
    tools.has('claude') && {
      ...
      command: `node "\${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/observe.js" # ${MOBILE_SPEC_OBSERVE_TAG}`,
    },
    tools.has('codex') && {
      value: readCodexHooks(targetPath),
      write: (value) => writeCodexHooks(targetPath, value),
      command: `node "$(git rev-parse --show-toplevel)/.codex/hooks/observe.js" # ${MOBILE_SPEC_OBSERVE_TAG}`,
    },
  ].filter(Boolean);
  ...
  for (const config of configs) {
    ...
    for (const eventName of eventNames) {
      const arr = Array.isArray(config.value.hooks[eventName])
        ? stripMobileSpecHooks(config.value.hooks[eventName])
        : [];
      arr.push({
