 ...
 * - 第一阶段不安装 slash command dispatcher，用户直接调用 Mobile Spec stage skills；
...
function loadManifestFile(file, label) {
  if (!fs.existsSync(file)) {
    throw new Error(`[mobile-spec] 清单不存在：${file}`);
  }
  ...
  if (current.extends) {
    ...
    if (!fs.existsSync(baseFile)) {
      throw new Error(`[mobile-spec] 基清单不存在：${baseFile}（来自 ${file}）`);
    }
    const base = parseYamlFile(baseFile);
    if (base.extends) {
      throw new Error(
        `[mobile-spec] 不支持嵌套 extends：${file} → ${baseFile} → ${base.extends}（基清单不得再 extends）`
      );
    }
...
function parseYamlFile(file) {
  ...
  try {
    ...
    const wrapped = new Error(
      `[mobile-spec] YAML 解析失败：${file} — ${err.message || err.reason || String(err)}`
    );
    wrapped.cause = err;
    throw wrapped;
  }
...
function installSkills(targetPath, skills) {
  ...
  for (const entry of skills) {
    ...
    if (!fs.existsSync(skillFile)) {
      throw new Error(
        `[mobile-spec] 文件型 skill 不存在：${skillFile}（清单条目 "${entry}"；外部 MCP 请改用 d-skills 安装）`
      );
    }
...
function installAgents(targetPath, platform, tools = ['claude']) {
  if (!ALLOWED_PLATFORMS.includes(platform)) {
    throw new Error(
      `[mobile-spec] 非法 platform: "${platform}"。可选值：${ALLOWED_PLATFORMS.join(', ')}`
    );
  }
...
/**
 * 删除 Mobile Spec 历史版本安装到业务项目的 OpenSpec skills / slash commands。
