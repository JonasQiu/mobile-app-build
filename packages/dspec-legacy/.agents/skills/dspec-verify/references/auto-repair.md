...
2. 仅当全部 blocking findings 都是确定性的 `implementation` 问题，且可在各自 `affectedFiles` 内安全修复时，coordinator 才自动修复。只要混有不确定、`environment`、`requirements` 或越界问题，就不得部分修复；直接按下表结束当前 invocation。不得修改 DSpec artifacts、补做 tasks 或顺手重构，subagent 始终只读。
3. 修改后把全部实际变更文件作为重复 `--file` 参数调用：
   ```bash
   dspec workflow hook --name recordVerifyRepair --stage verify --change <change> --file <changed-file> --json
   ```
...
当前 invocation 最多自动修复 2 次，即“初次 Verify → 修复 1 → 复验 → 修复 2 → 最终复验”。该上限只约束本次调用；用户再次调用 `dspec-verify` 才创建新 invocation 并重新计数。若修复成功记录后尚未产生合法复验结果便发生 profile、subagent 或 `postNode` 中断，按主流程执行 `abortVerifyInvocation`，不得用旧失败结果 `postStage`。
