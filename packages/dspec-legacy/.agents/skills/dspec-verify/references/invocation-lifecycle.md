...
1. 记录实际失败步骤与原因，执行：
   ```bash
   dspec workflow hook --name abortVerifyInvocation --stage verify --change <change> --failure-step <step> --reason <失败原因> --json
   ```
2. hook 成功后确认 invocation 为 `aborted`、`checksUpdated: false`、`currentUpdated: false`；已归档时还必须是 `workflowImpact: none`。
3. 停止当前调用，不执行 `postStage`，不回滚已发生的产品文件变化。用户之后再次调用 `dspec-verify` 才创建新 invocation。
