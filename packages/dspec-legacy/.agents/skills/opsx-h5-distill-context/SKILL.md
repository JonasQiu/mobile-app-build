...
description: 从现有项目代码与文档中蒸馏初始上下文，批量填充 docs/context/ 下的 6 个上下文文件（glossary / route-map / api-map / state-model / webview-bridge / tracking）。用于项目首次接入 dspec、或 context 文档严重过时需重建时。
category: Bootstrap
...
- 后续每次 change archive 后运行 dspec-sync-context 增量维护
```
