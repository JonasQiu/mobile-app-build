---
name: dspec-proposal
description: 创建或继续 DSpec change，生成 proposal 与 specs。用于需求提案和规格阶段；不要用于技术设计或编码。
license: MIT
---

# DSpec Proposal

完成 propose stage。新建 change 必须有望岳需求或 Cooper PRD 链接；继续已有 change 复用已记录来源。
...
1. 执行 `openspec status --change <change> --json` 判断 change：
   - 成功表示继续已有 change，同时读取 `dspec workflow status --change <change> --json`，只使用其 `storage.*` 路径。
   - 只有明确 change not found 才进入新建；其他错误停止。
   - “继续”分支不得执行 `preNew`、`openspec new change` 或 `postNew`，不得覆盖已有 requirement source。
2. 已有 change 按状态幂等恢复：
   - workflow propose 与 OpenSpec proposal/specs 都 done 时不重写，调用 `dspec workflow next --change <change> --json` 后返回。
...
10. 执行 `postStage --stage proposal`；只有顶层 `ok: true` 才调用 `dspec workflow next --change <change> --json`。

## 输出

总结 change、来源与读取路径、望岳 ID、已读需求与设计稿截图、未读链接类 Design 输入、Proposal 状态、Specs、rules、agentActions、gate 结果与下一阶段。不得要求 RD 逐份确认 Specs；失败时列出原因、影响和恢复步骤。
