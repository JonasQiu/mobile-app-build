/**
 * dspec init [path] [-p|--platform <h5|native>] [-f|--force] [--tools <tools>]
 *
 * 步骤序列：
...
async function promptPlatform(opts = {}) {
  ...
  if (!isTTY) {
    stderr.write(
      `[dspec] 未指定 platform，且当前为非交互环境（非 TTY）。\n` +
...
async function confirmOverwriteAll(existing, opts = {}) {
  ...
  if (force) {
    stdout.write(`[dspec] --force 已指定，将覆盖以下已存在文件：\n`);
  ...
  if (!isTTY) {
    stderr.write(
      `[dspec] 检测到目标项目已存在以下文件，init 将覆盖它们：\n` +
      existing.map(f => `        - ${f}\n`).join('') +
...
/**
 * 解析参数：dspec init [path] [-p|--platform <h5|native>] [-f|--force] [--tools <tools>]
...
function parseInitArgs(args) {
  ...
  if (platform !== null && !ALLOWED_PLATFORMS.includes(platform)) {
    console.error(
      `[dspec] 非法 --platform 值: "${platform}"。可选值：${ALLOWED_PLATFORMS.join(', ')}`
    );
    process.exit(1);
  }
  ...
  if (tools !== null) {
    ...
    if (invalid.length > 0) {
      console.error(
        `[dspec] 非法 --tools 值: "${invalid.join(', ')}"。可选值：${ALLOWED_TOOLS.join(', ')}`
      );
      process.exit(1);
    }
  }
...
async function cmdInit(args, _opts = {}) {
  ...
  console.log(`\ndspec init  ${platform} · ${targetPath}\n`);

  // 1a. 把包内 schema 暂存到 ~/.dspec/schemas/<name>/
  stageSchemaToDSpecDir(schemaName);
  const schemaDSpecDir = path.join(getDSpecSchemasDir(), schemaName);
  console.log(`  ✓ staged    → ${schemaDSpecDir}`);

  // 1b. 从 ~/.dspec/ 安装到 openspec 用户级真实目录（~/.local/share/openspec/schemas/<name>/）
  ...
  if (migration.migrated.length > 0) {
    console.log(`  ✓ migrated  → ~/.dspec/workflow/ (${migration.migrated.length} files)`);
  }
  if (migration.conflicts.length > 0) {
    console.log(`\n⚠️  ${migration.conflicts.length} 个旧 workflow 文件与用户目录状态冲突，已保留原文件`);
  }

  // 6. 安装公司 eval 监控插件(DSPEC_SKIP_EVAL=1 跳过;失败降级不阻断)
  if (process.env.DSPEC_SKIP_EVAL !== '1') {
    installMonitor([]);
  }

  console.log('\n初始化完成，重启 IDE 后 DSpec skills 生效。');
  printDSkillsHint();
}
