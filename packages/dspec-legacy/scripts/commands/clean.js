 ...
 *
 * 清理两处 schema 存储：
 *   1. ~/.dspec/schemas/<name>/          — dspec 暂存层
...
const { getUserSchemasDir, getDSpecSchemasDir, PLATFORM_TO_SCHEMA } = require('../schema/register');
...
function cleanDir(dest, label) {
  ...
  try {
    ...
    console.log(`[dspec] ${label} 未找到，跳过（${dest}）`);
    return;
  }

  if (stat.isSymbolicLink()) {
    console.warn(`[dspec] 警告：${dest} 是 symlink，跳过清理，请手动删除。`);
    return;
  }

  fs.rmSync(dest, { recursive: true, force: true });
  console.log(`[dspec] ✅ ${label} 已清理：${dest}`);
}
...
function cmdClean() {
  ...
  for (const name of schemaNames) {
    // 1. 清理 dspec 暂存层
    cleanDir(path.join(getDSpecSchemasDir(), name), `staged(${name})`);
