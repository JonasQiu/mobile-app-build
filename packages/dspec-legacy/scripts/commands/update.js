...
/**
 * 解析参数：dspec update [path] [-p|--platform <name>] [--tools <tools>]
...
function parseUpdateArgs(args) {
  ...
  if (explicitPlatform !== null && !ALLOWED_PLATFORMS.includes(explicitPlatform)) {
    console.error(
      `[dspec] 非法 --platform 值: "${explicitPlatform}"。可选值：${ALLOWED_PLATFORMS.join(', ')}`
    );
    process.exit(1);
  }
...
async function resolvePlatform(targetPath, explicitPlatform) {
  ...
  console.error(
    `[dspec] 无法从 ${configPath} 推断具体子平台，请选择或先运行 \`dspec init\`。`
  );
...
async function cmdUpdate(args, _opts = {}) {
  ...
  console.log(`\ndspec update  ${platform} · ${targetPath}\n`);

  // 1a. 把包内最新 schema 暂存到 ~/.dspec/schemas/<name>/
  ...
  // 5. 把旧版项目内 workflow sidecar 迁到 ~/.dspec/workflow/
  ...
  // 安装/刷新公司 eval 监控插件(DSPEC_SKIP_EVAL=1 跳过;失败降级不阻断)
  if (process.env.DSPEC_SKIP_EVAL !== '1') {
    installMonitor([]);
  }

  console.log('\n更新完成，重启 IDE 后 DSpec skills 生效。');
  printDSkillsHint();
}
