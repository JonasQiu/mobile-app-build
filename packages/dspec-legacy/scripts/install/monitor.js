...
/**
 * dspec install-monitor —— 安装公司 AI coding eval 监控插件
 *
 * 移植自 hyper/lib/installMonitor.js,默认 workflow 改为 "DSpec"。
 ...
 *
 * 失败一律 warn 降级,不阻断 dspec init/update 主流程。
 * 路径解析尊重 DSPEC_HOME_OVERRIDE(测试隔离,与 register-schema 一致)。
 */
...
const EVAL_DEFAULT_WORKFLOW = "DSpec";

/** 测试隔离:DSPEC_HOME_OVERRIDE 可重定向 home(与 register-schema 一致)。 */
function getHome() {
  return process.env.DSPEC_HOME_OVERRIDE || os.homedir();
}
...
function patchPluginEvalCollectHook() {
  ...
  for (const entry of fs.readdirSync(cacheDir)) {
    ...
    if (changed) {
      ...
      console.log(`  Patched eval-collect hook to use dspec wrapper (${hooksPath})`);
    }
  }
}
...
function printInstallMonitorHelp() {
  console.log(`
Usage: dspec install-monitor [options]
...
wrappers on PATH. Both default to the DSpec workflow via AI_CODING_EVAL_WORKFLOW.
...
Examples:
  dspec install-monitor
  dspec install-monitor --skip-eval
`);
}
