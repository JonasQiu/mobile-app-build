/**
 * dspec schema 注册工具函数。
 * 供 `dspec init` / `dspec update` / `dspec clean` 调用。
 *
 * 架构说明：
 ...
 *
 * 测试隔离：设置 DSPEC_HOME_OVERRIDE 可将路径重定向到临时目录，
 *   不污染真实用户目录（等价于把 os.homedir() 替换为 DSPEC_HOME_OVERRIDE）。
 */
...
/**
 * 读取当前发布包版本，作为 schema 的 DSpec 发布归属版本。
 * OpenSpec 自身的 schema.version 必须保持正整数，两者不能复用同一字段。
 */
function getDSpecVersion() {
  try {
...
/**
 * 把 schema.yaml 中的 dspecVersion 占位符替换为当前 CLI 版本。
 * 对缺少该字段的旧 schema 也会在整数 version 后补写，保证升级后安装产物可追踪。
 */
function materializeDSpecVersion(schemaDir) {
  const schemaFile = path.join(schemaDir, 'schema.yaml');
  if (!fs.existsSync(schemaFile)) return false;

  const versionLine = `dspecVersion: ${JSON.stringify(getDSpecVersion())}`;
  const original = fs.readFileSync(schemaFile, 'utf8');
  let content = original;
  if (/^dspecVersion:\s*.*$/m.test(content)) {
    content = content.replace(/^dspecVersion:\s*.*$/m, versionLine);
...
function platformToSchema(platform) {
  ...
  if (!name) {
    throw new Error(
      `[dspec] 未知 platform: "${platform}"。可选值：${Object.keys(PLATFORM_TO_SCHEMA).join(', ')}`
    );
  }
  return name;
}
 ...
 *
 * 测试隔离：DSPEC_HOME_OVERRIDE 替换 os.homedir()，XDG_DATA_HOME 优先级更高。
 */
function getUserSchemasDir() {
  ...
  // DSPEC_HOME_OVERRIDE 仅供测试使用：覆盖 os.homedir()，不污染真实用户目录。
  const homeDir = process.env.DSPEC_HOME_OVERRIDE || os.homedir();
...
/**
 * 返回 dspec 暂存层根目录：~/.dspec/schemas/
 *
 * 所有 platform schema 暂存于此，作为 openspec 用户级安装的来源。
 * 测试隔离：DSPEC_HOME_OVERRIDE 替换 os.homedir()。
 */
function getDSpecSchemasDir() {
  const homeDir = process.env.DSPEC_HOME_OVERRIDE || os.homedir();
  return path.join(homeDir, '.dspec', 'schemas');
}

/**
 * 把包内 schema 暂存到 ~/.dspec/schemas/<schemaName>/（每次覆盖刷新）。
 * init / update 时首先调用此步骤。
 *
 * @param {string} schemaName  如 'h5-sdd'、'native-sdd'
 * @returns {{ dest: string }}
 */
function stageSchemaToDSpecDir(schemaName = SCHEMA_NAME) {
  const dest = path.join(getDSpecSchemasDir(), schemaName);
  const src = getPackageSchemaDir(schemaName);
  removePath(dest);
  copyDirSync(src, dest);
  materializeDSpecVersion(dest);
  return { dest };
}

/**
 * 返回 schema 安装来源目录（两级优先级）：
 *   1. ~/.dspec/schemas/<schemaName>/  — 暂存层（优先）
...
function getSchemaSourceDir(schemaName = SCHEMA_NAME) {
  const staged = path.join(getDSpecSchemasDir(), schemaName);
 ...
 * 来源优先取 ~/.dspec/schemas/<schemaName>/（暂存层），
