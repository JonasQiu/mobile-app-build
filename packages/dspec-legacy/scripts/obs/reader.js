 ...
 *
 * 合并两套可观测数据源，输出汇总视图：
 *   - observe 被动层：~/.dspec/obs/<hash>/events.jsonl（工具级/产物级事件）
 *   - monitor 主动层：~/.dspec/monitor/state/repos/<repoKey>/requirements/*.json（phase 区间/validate 状态）
 *
 ...
 * 路径解析复用 observe.js 的 getObsHome / monitor.js 的 dataRoot / stateRoot，统一吃 DSPEC_HOME_OVERRIDE。
 */
...
/**
 * 遍历 ~/.dspec/monitor/state/repos/<repoKey>/requirements/*.json，
