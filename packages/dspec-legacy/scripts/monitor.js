...
/**
 * dspec monitor —— 公司 eval-emit 的薄适配器
 *
 * DSpec 只在 workflow 边界调用公司的五类信号。传输、重试、产物指纹比对和通过/返工
 * 判定均由公司链路负责；本模块不实现 outbox、自动重试或 validate 去重。eval-emit
 * 失败只输出 EVAL_EMIT_FAILED warning，不阻断 SDD workflow；DSPEC_EVAL_SKIP=1 可禁用。
 * 本地状态仅用于绑定 requirement、避免明显重复 start，以及为本地看板保留区间。
 *
 * 调用入口统一由 `dspec workflow hook` 持有，stage skill 不直接编排信号。
 *
 ...
 *
 * 状态落点:`~/.dspec/monitor/state/`(DSPEC_HOME_OVERRIDE 可隔离),与事件落点(项目级)分离:
...
const EVENT_SOURCE = "dspec-monitor";
const DEFAULT_WORKFLOW = "DSpec";
...
/** dspec 状态根:~/.dspec/monitor/(DSPEC_HOME_OVERRIDE 可重定向,与 register-schema 一致)。 */
function dataRoot() {
  const home = process.env.DSPEC_HOME_OVERRIDE || os.homedir();
  return path.join(home, ".dspec", "monitor");
}
...
function currentSessionId() {
  return (
    process.env.DSPEC_MONITOR_SESSION_ID ||
 ...
 * DSPEC_EVAL_SKIP=1 禁用;eval-emit 不在 PATH 时提示跑 `dspec install-monitor`。
 */
function runEvalEmit(argv) {
  if (process.env.DSPEC_EVAL_SKIP === "1") {
    return false;
  }
  try {
    ...
    const detail =
      error && error.code === "ENOENT"
        ? "eval-emit not on PATH (run `dspec install-monitor` to install wyc-ai-coding-insight)"
...
function printHelp() {
  console.log(`
Usage: dspec monitor <command> [options]
